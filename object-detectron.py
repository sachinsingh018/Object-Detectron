...
def object_detection_image():
    ...
    st.subheader("""
    You would be required to give us an image which we will be later outputted with boxes bounding the detected and recognized objects for the same. Provided that the object belongs to our pre specified list of 80 objects. \U0001F642
    Feel free to move around the confidence bar for detection of object's process's thresholds and there is analytical data available too if in case the user wishes to view it.
    """)
...
def object_detection_video():
    ...
    st.subheader("""
    This part requires a video and outputs the same video with bounding boxes created around the objects in the video along with their respective labels for the same. 
    """
    )
...
def main():
    new_title = '<p style="font-size: 42px;">Welcome to this application! Well done on the task of following all the instructions in the readme file \U0001f600 </p>'
    read_me_0 = st.markdown(new_title, unsafe_allow_html=True)

    read_me = st.markdown("""
    The project utilizes YOLOv3 for object detection along with streamlit for the UI implementation.
    It can detect a wide range of objects and perform an efficient classification for the same.
    The YOLO algorithm utilizes the open source darknet Neural Network architecture for its implementation.

    Also, YOLO stands for "You look only once".
    ...
    Feel free to have a look at the same"""
    )
    st.sidebar.title("Choose from the Dropdown")
    choice  = st.sidebar.selectbox("Select",("About the detectron","Detect for images","Detect for videos"))
    #["Show Instruction","Landmark identification","Show the #source code", "About"]
    
    if choice == "Detect for images":
        #st.subheader("Object Detection")
        read_me_0.empty()
        read_me.empty()
        #st.title('Object Detection')
        object_detection_image()
    elif choice == "Detect for videos":
        read_me_0.empty()
        read_me.empty()
